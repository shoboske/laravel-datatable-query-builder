...
{
    public function __invoke($declaredRelationship, $relationships, $with = [], $sortDirection = 'asc')
    {
                ...
                foreach ($belongsToMany as $name) {
                    if ($tableName === $name) {
                        ...
                        if (isset($relationship['order_by'])) {
                            $columnName = $relationship['order_by'];

                            $with[$name] = function ($q) use ($columnName, $sortDirection) {
                                $q->orderBy($columnName, $sortDirection);
                            };
                        }
                    }
                }
